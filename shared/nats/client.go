...
// ErrNoReply is returned when a first-reply request times out without a response.
var ErrNoReply = errors.New("no nats reply")
...
// Msg is a NATS message delivered to subscription handlers.
type Msg = natsgo.Msg
...
func New(url, name string, logger *slog.Logger) (*Client, error) {
	...
	natsConn, err := natsgo.Connect(
		...
		natsgo.DisconnectErrHandler(func(_ *natsgo.Conn, err error) {
			if err != nil {
				client.logger.Warn("nats disconnected", "component", "nats", "client", client.name, "error", err)

				return
			}
			client.logger.Warn("nats disconnected", "component", "nats", "client", client.name)
		}),
		natsgo.ReconnectHandler(func(natsConn *natsgo.Conn) {
			client.logger.Info("nats reconnected", "component", "nats", "client", client.name, "url", natsConn.ConnectedUrl())
		}),
		natsgo.ClosedHandler(func(natsConn *natsgo.Conn) {
			if err := natsConn.LastError(); err != nil {
				client.logger.Warn("nats closed", "component", "nats", "client", client.name, "error", err)

				return
			}
			client.logger.Info("nats closed", "component", "nats", "client", client.name)
		}),
		natsgo.ErrorHandler(func(_ *natsgo.Conn, sub *Subscription, err error) {
			...
			client.logger.Warn("nats async error", "component", "nats", "client", client.name, "subject", subject, "error", err)
		}),
	)
	if err != nil {
...
func (c *Client) FirstReply(subject string, payload []byte) (*Msg, error) {
	...
	// registered so the publishing cannot outrun the inbox setup.
	if err := c.Flush(defaultFlushTimeout); err != nil {
