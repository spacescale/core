...
func (s *AppService) CreateApp(ctx context.Context, ownerUserID, workspaceID, projectID string, p CreateAppParams) (App, error) {
	...
	for i := 0; i < maxSlugRetries; i++ {
		...
		row, createErr := txQueries.CreateApp(ctx, pgstore.CreateAppParams{
		...
		if err := txQueries.CreateQueuedDeployment(ctx, pgstore.CreateQueuedDeploymentParams{
			AppID:       row.ID,
			ImageRef:    imageRef,
			RuntimePort: runtimePort,
		}); err != nil {
		...
		if hasRegistryCredential {
			if err := txQueries.UpsertAppRegistryCredential(ctx, pgstore.UpsertAppRegistryCredentialParams{
				AppID:                row.ID,
		...
		for _, env := range envVars {
			...
			if err := txQueries.CreateAppEnvVar(ctx, pgstore.CreateAppEnvVarParams{
				AppID:          row.ID,
				Key:            env.Key,
...
func (s *AppService) authorizeOwnedProject(ctx context.Context, ownerUserID, workspaceID, projectID string) (uuid.UUID, error) {
	...
	projectRow, err := s.queries.GetProjectByIDAndOwnerUserID(ctx, pgstore.GetProjectByIDAndOwnerUserIDParams{
		ID:          projectUUID,
		OwnerUserID: ownerUUID,
	})
	if err != nil {
...
func (s *AppService) resolveRegistryCredential(ctx context.Context, projectID uuid.UUID, rawCredentialID string) (uuid.UUID, bool, error) {
	...
	_, err := s.queries.GetRegistryCredentialByIDAndProjectID(ctx, pgstore.GetRegistryCredentialByIDAndProjectIDParams{
		ID:        credentialID,
		ProjectID: projectID,
	})
	if err != nil {
...
func appFromRow(r pgstore.App) App {
	return App{
		ID:          r.ID.String(),
