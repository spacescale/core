...
func TestDirectiveTarget(t *testing.T) {
	if got := directiveTarget(&controlv1.ControlDirective{Target: &controlv1.ControlDirective_Node{Node: &controlv1.NodeDirective{}}}); got != "node" {
		t.Fatalf("expected node target, got %s", got)
	}
	if got := directiveTarget(&controlv1.ControlDirective{Target: &controlv1.ControlDirective_Agent{Agent: &controlv1.AgentDirective{}}}); got != "agent" {
		t.Fatalf("expected agent target, got %s", got)
	}
	if got := directiveTarget(&controlv1.ControlDirective{Target: &controlv1.ControlDirective_Workload{Workload: &controlv1.WorkloadDirective{}}}); got != "workload" {
		t.Fatalf("expected workload target, got %s", got)
	}
	if got := directiveTarget(&controlv1.ControlDirective{}); got != "unknown" {
