...
func (c *Client) runSession(ctx context.Context) error {
	...
	healthResp, err := client.Health(healthCtx, &controlv1.HealthRequest{Caller: c.cfg.Agent.Caller, RequestId: c.nextRequestID("health")})
	cancelHealth()
	if err != nil {
	...
	if err := stream.Send(&controlv1.AgentSessionRequest{
...
) (controlv1.ControlPlane_OpenSessionClient, error) {
	type result struct {
		stream controlv1.ControlPlane_OpenSessionClient
		err    error
	}
...
func (c *Client) recvSessionResponseWithTimeout(
	...
	stream controlv1.ControlPlane_OpenSessionClient,
	timeout time.Duration,
) (*controlv1.AgentSessionResponse, error) {
	type result struct {
		resp *controlv1.AgentSessionResponse
		err  error
	}
...
func (c *Client) recvLoop(ctx context.Context, stream controlv1.ControlPlane_OpenSessionClient, errCh chan<- error) {
	for {
		resp, err := stream.Recv()
		if err != nil {
			if errors.Is(err, io.EOF) {
...
func directiveTarget(d *controlv1.ControlDirective) string {
	switch {
	case d.GetNode() != nil:
		return "node"
	case d.GetAgent() != nil:
