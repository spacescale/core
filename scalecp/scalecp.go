// Package scalecp starts the SpaceScale control-plane process.
package scalecp
...
// Run starts the scalecp control plane and blocks until the context is canceled or startup fails.
func Run(ctx context.Context) error {
	cfg, err := config.LoadScalecp()
	if err != nil {
		return fmt.Errorf("load scalecp config: %w", err)
	}
	...
	natsClient, err := nats.New(cfg.NATSURL, "scalecp", log)
	if err != nil {
	...
	group.Go(func() error {
		log.Info("scalecp listening", "component", "scalecp", "addr", cfg.ListenAddr())
